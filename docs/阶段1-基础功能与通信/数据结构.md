...
| `TASK_CMD` | 任务下发命令 |
| `JOY_CONTROL_CMD` | 摇杆控制命令 |
| `SET_MARKER_CMD` | 设置标记命令 |
| `CHARGE_CMD` | 充电命令 |
...
# 创建摇杆控制命令消息信封
create_joy_control_cmd_envelope(cmd_id: str, robot_id: str, joy_control_cmd: joyControlCmd) -> CommandEnvelope
```
...
| 字段 | 类型 | 描述 |
|------|------|------|
| `inspection_task_list` | `List[Task]` | 任务列表 |
...
#### ArriveServicePointInfo - 到达服务点信息
| 字段 | 类型 | 描述 |
|------|------|------|
...
#### ArriveServePointDataJson - 到达服务点数据
| 字段 | 类型 | 描述 |
|------|------|------|
| `positionInfo` | `PositionInfo` | 位置信息 |
| `taskInfo` | `TaskInfo` | 任务信息 |
| `arriveServePointInfo` | `ArriveServicePointInfo` | 到达服务点信息 |
...
# 创建到达服务点消息
create_arrive_serve_point_message(
    msg_id: str, robot_id: str, position_info: PositionInfo,
    task_info: List[TaskInfo], arrive_service_point_info: ArriveServicePointInfo
) -> MessageEnvelope
```
...
5. **工厂模式使用**：推荐使用工厂方法创建消息和命令，确保类型安全和代码一致性
...
3. **摇杆控制流程**：
   - 客户端发送摇杆控制命令 → 创建 `joyControlCmd` → 包装为 `CommandEnvelope` → 发送给机器人
   - 机器人执行移动指令 → 创建状态消息上报 → 客户端接收更新
