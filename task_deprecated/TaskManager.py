...
from task_deprecated.TaskScheduler import TaskScheduler
from dataModels.TaskModels import StationConfig, Station, OperationMode, OperationConfig
import logging
class TaskManager:
    """任务管理器 - 主控制器"""
    ...
    def _on_task_start(self, station: Station):
        """任务开始回调"""
        self.logger.info(f"任务开始: {station.station_id}")
        # 可以在这里发送通知或更新UI
    
    def _on_task_complete(self, task: Station):
        """任务完成回调"""
        self.logger.info(f"任务完成: {task.station_id}")
        # 可以在这里发送通知或更新UI
    
    def _on_task_failed(self, task: Station):
        """任务失败回调"""
        self.logger.error(f"任务失败: {task.station_id}")
        # 可以在这里发送通知或更新UI
